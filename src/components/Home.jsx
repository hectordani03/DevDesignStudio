...
export const Home = () => {
  ...
  const [homeImage, setHomeImage] = useState(theme === 'dark' ? SvgHome2 : SvgHome);

  // Actualizar la clase en el html cuando cambia el tema y la imagen
  useEffect(() => {
    if (theme === 'dark') {
      document.querySelector('html').classList.add('dark');
      setHomeImage(SvgHome2);
    } else {
      document.querySelector('html').classList.remove('dark');
      setHomeImage(SvgHome);
    }
